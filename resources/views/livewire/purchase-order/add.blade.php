        ...
        <div class="modal-dialog modal-dialog-centered vendor-draggable" style="max-width: 500px;">
            <div class="modal-content" style="background: #fffaf0; border: 1px solid #999; font-size: 13px;">
                <div class="modal-header py-2 px-3 vendor-draggable-header"
                    ...
                    <button type="button" class="btn btn-link text-danger p-0" style="font-size: 13px;"
    onclick="event.stopPropagation(); @this.call('closeVendorAlertPopup');">Close</button>
                </div>
...
<style>
    ...
    /* Common draggable modal styles */
    .alert-draggable, .profile-draggable, .vendor-draggable {
        position: fixed;
        margin: 0;
        z-index: 1050;
        pointer-events: auto;
    ...
    /* Vendor Alert Modal - Lower position */
    .vendor-draggable {
        top: 56% !important;
        left: 25% !important;
        transform: translate(0px, 0px) !important;
    ...
    .alert-draggable-header, .profile-draggable-header, .vendor-draggable-header {
        cursor: move;
    }
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Initialize draggable for all modals using interact.js
    function initDraggableModal(dialogClass, handleClass, modalName) {
        const elements = document.querySelectorAll(dialogClass);
            ...
            interact(element).draggable({
                ...
                listeners: {
                    ...
                    move(event) {
                        ...
                        // Store position
                        if (modalName) {
                            modalPositions[modalName] = { x: x, y: y };
                        }
                    }
                }
            });
    ...
    function setInitialModalPosition(modalClass, modalName, topPos, leftPos) {
        const modal = document.querySelector(modalClass);
        if (modal) {
            if (modalPositions[modalName]) {
                modal.style.transform = `translate(${modalPositions[modalName].x}px, ${modalPositions[modalName].y}px)`;
                modal.setAttribute('data-x', modalPositions[modalName].x);
                modal.setAttribute('data-y', modalPositions[modalName].y);
            } else {
                modal.style.top = topPos;
                modal.style.left = leftPos;
    ...
    initDraggableModal('.alert-draggable', '.alert-draggable-header', 'alert');
    initDraggableModal('.profile-draggable', '.profile-draggable-header', 'profile');
    initDraggableModal('.vendor-draggable', '.vendor-draggable-header', 'vendor');

    // Livewire event listeners
    document.addEventListener('livewire:init', () => {
        Livewire.on('showAlertPopup', () => {
            setTimeout(() => setInitialModalPosition('.alert-draggable', 'alert', '8%', '25%'), 50);
        });

        Livewire.on('showProfilePopup', () => {
            setTimeout(() => setInitialModalPosition('.profile-draggable', 'profile', '32%', '35%'), 50);
        });
        
        Livewire.on('showVendorAlertPopup', () => {
            setTimeout(() => setInitialModalPosition('.vendor-draggable', 'vendor', '56%', '25%'), 50);
        });
    });

    // Initial positioning if modals are already visible
    if (document.querySelector('.alert-draggable') && document.querySelector('.alert-draggable').closest('.modal.show')) {
        setInitialModalPosition('.alert-draggable', 'alert', '8%', '25%');
    }
    if (document.querySelector('.profile-draggable') && document.querySelector('.profile-draggable').closest('.modal.show')) {
        setInitialModalPosition('.profile-draggable', 'profile', '32%', '35%');
    }
    if (document.querySelector('.vendor-draggable') && document.querySelector('.vendor-draggable').closest('.modal.show')) {
        setInitialModalPosition('.vendor-draggable', 'vendor', '56%', '25%');
    }
});

// For alert types update
document.addEventListener('livewire:init', function () {
    Livewire.on('alert-types-updated', () => {
        setTimeout(() => {
            const alertTypes = @this.alertTypes || [];
            document.querySelectorAll('[wire\\:model="alertTypes"]').forEach(checkbox => {
                const shouldBeChecked = alertTypes.includes(checkbox.value);
